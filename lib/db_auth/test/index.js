...
var username2 = 'user2_' + Date.now();
var username3 = 'user3_' + Date.now();
var email = 'user_' + Date.now() + '@foo.com';
var email2 = 'user2_' + Date.now() + '@foo.com';
var email3 = 'user3_' + Date.now() + '@foo.com';
var user = null;
...
async.waterfall([
    ...
    // update email
    function(callback){
        dbAuth.updateEmail({id: user.id, email: email3},
                           function(err, response){
            if( err ){ return callback(err); }
            assert(response.status === 'success', 'Could not update email');
            callback();
        })
    },
    ...
    function(callback){
        dbAuth.getUser({
            ...
            if( err ){ return callback(err); }
            assert(response.status === 'success', 'Could not get user by updated email');
            assert(response.user.id === user.id, 'User does not match update user with udpate email');
            callback()
        });
    },
    ...
    // update username
    function(callback){
        dbAuth.updateUsername({id: user.id, username: username3},
                           function(err, response){
            if( err ){ return callback(err); }
            assert(response.status === 'success', 'Could not update username');
            callback();
        })
    },
    ...
    function(callback){
        dbAuth.getUser({
            ...
            if( err ){ return callback(err); }
            assert(response.status === 'success', 'Could not get user after updating username');
            assert(response.user.id === user.id, 'User does not match user with udpated username');
            callback()
        });
    },

    // try to update email to use existing email
    function(callback){
        dbAuth.updateEmail({id: user.id, email: email2},
                           function(err, response){
            if( err ){ return callback(err); }
            assert(response.errorCode === 'emailClaimed', 'Should not have updated email');
            callback();
        })
    },

    // try to update username to use exising username
    function(callback){
        dbAuth.updateUsername({id: user.id, username: username2},
                           function(err, response){
            if( err ){ return callback(err); }
            assert(response.errorCode === 'usernameClaimed', 'Should not have updated username');
            callback();
        })
    },
    ...
    function(callback){
        if( !testDelete ){ return callback(); }
        dbAuth.delete({email: email3}, function(err, response){
            if( err ){ return callback(err); }
            assert(response.status === 'success');
