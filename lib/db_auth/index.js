...
const errors = {
    ...
    emailClaimed: 'The email is already taken',
...
module.exports = function(options, callbackConstructor){
    ...
    /**
     * @param {string}  options.id
     * @param {string} options.email  new email
     */
    this.updateEmail = function(options, callback){
        ...
        this.driver.getUser({id: options.id}, function(err, user){
            if( err ){ return callback(err); }
            if( !user ){ return getResponse('noUser', callback); }
            self.driver.emailIsClaimed(options, function(err, isClaimed){
                if( err ){ return callback(err); }
                if( isClaimed ){ return getResponse('emailClaimed', callback); }
                self.driver.updateEmail(options, function(err){
                    if( err ){ return callback(err); }
                    return getResponse(null, callback);
                });
            });
    ...
    /**
     * @param {string}  options.id
     * @param {string} options.username  new username
     */
    this.updateUsername = function(options, callback){
        ...
        this.driver.getUser({id: options.id}, function(err, user){  
            if( err ){ return callback(err); }
            if( !user ){ return getResponse('noUser', callback); }
            self.driver.usernameIsClaimed(options, function(err, isClaimed){
                if( err ){ return callback(err); }
                if( isClaimed ){
                    return getResponse('usernameClaimed', callback);
                }
                self.driver.updateUsername(options, function(err){
                    if( err ){ return callback(err); }
                    return getResponse(null, callback);
                });
            });
